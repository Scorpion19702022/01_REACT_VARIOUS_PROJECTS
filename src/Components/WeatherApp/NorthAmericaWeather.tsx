...
import styles from './Styles/NorthAmericaWeather.module.css'
...
import NorthAmMap from './assets/NorthAmer.png'

import WomanWeather from './assets/back2.png'
import NorthAmContext from './Context/NorthAmContext'

const NorthAmericaWeather = () => {
	const { refresh, citiesNorthAm, northAmCityTime } = useContext(NorthAmContext)
	...
	const citiesOfNorthAm = citiesNorthAm.map((city, index) => (
		<div key={index} className={styles.box_cities}>
			<div className={styles.top}>
				<h4 className={styles.name_city}>{city.city === 'Hyattsville' ? 'Washington' : city.city}</h4>
			</div>
	...
	return (
		<section className={styles.wrapper}>
			...
			<div className={styles.box_locale_time}>
				<h4 className={styles.city_locale_time}>Czas: New York</h4>
				<span className={styles.locale_time}>{northAmCityTime}</span>
			</div>
...
export default NorthAmericaWeather
